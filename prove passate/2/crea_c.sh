...
then
		...
		echo $'\t' '//Allocazione di un array di pipe per la comunicazione fra filgi' >> $nomefile
		echo $'\t' 'npipe=nfigli-1;' >> $nomefile
		echo $'\t' 'pipes= malloc(sizeof(pipe_t)*npipe);' >> $nomefile
	    ...
	    echo $'\t' $'\t' $'\t' $'\t' '}' >> $nomefile
	  	...
	  	echo $'\t' $'\t' $'\t' 'figlio(argv[i]);' >> $nomefile
	  	echo $'\t' $'\t' $'\t' 'return;' >> $nomefile
		...
		echo $'\t' $'\t' 'if (WEXITSTATUS(status)==255) {' >> $nomefile
        echo $'\t' $'\t' $'\t' 'zprintf(1,"Errore in status\n");' >> $nomefile
        echo $'\t' $'\t' '} else {' >> $nomefile
